...
public class CustomPizzaRepositoryImpl implements CustomPizzaRepository{
    ...
    public List<Pizza> findAllByExample(Pizza pizzaExample) {
        ...
        StringBuilder queryBuilder = new StringBuilder("select p from Pizza p where cp.id = p.id ");

        if(StringUtils.isNotEmpty(pizzaExample.getDescrizione())){
            whereClauses.add(" p.descrizione like :descrizione");
            paramaterMap.put("descrizione", "%" + pizzaExample.getDescrizione() + "%");
        }
        if(StringUtils.isNotEmpty(pizzaExample.getIngredienti())){
            whereClauses.add(" p.ingredienti like :ingredienti");
            paramaterMap.put("ingredienti", "%" + pizzaExample.getIngredienti() +"%");
        }
        if(pizzaExample.getPrezzo() != null){
            whereClauses.add(" p.prezzo >= :prezzo");
            paramaterMap.put("prezzo",pizzaExample.getPrezzo());
        }
        if(pizzaExample.getAttivo() != null){
            whereClauses.add(" p.attivo = :attivo");
            paramaterMap.put("attivo",pizzaExample.getAttivo());
        }
